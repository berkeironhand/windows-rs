...
impl<'a, 'b: 'a> Encoder<'a, 'b> {
    pub(super) fn encode_callback(&mut self, ty: &Callback) -> Result<(), Error> {
        ...
        let return_type = self.encode_return_type(&ty.sig.output)?;

        if let Some(variadic) = &ty.sig.variadic {
            return self.err(
                variadic,
