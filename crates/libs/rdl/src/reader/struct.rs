...
impl<'a, 'b: 'a> Encoder<'a, 'b> {
    pub(super) fn encode_struct(&mut self, ty: &Struct) -> Result<(), Error> {
        let type_def =
            self.encode_struct_or_union(&ty.name.to_string(), ty.winrt, false, &ty.fields)?;
    ...
    pub(super) fn encode_struct_or_union(
        &mut self,
        item_name: &str,
        winrt: bool,
        is_union: bool,
