...
impl<'a, 'b: 'a> Encoder<'a, 'b> {
    pub(super) fn encode_const(&mut self, ty: &Const) -> Result<(), Error> {
        let name = ty.name.to_string();
        let t = self.encode_type(&ty.ty)?;

        match &t {
            windows_metadata::Type::Name(tn) if tn == ("System", "Guid") => {
                self.encode_const_guid(&t, ty, &name)?;
            }
            _ => self.encode_const_value(&t, ty, &name)?,
        }
